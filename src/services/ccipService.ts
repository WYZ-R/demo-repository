...
class CCIPService {
   ...
   * @returns 资产地址
   */
  getAssetAddress(chainId: ChainId, assetType: string): string | PublicKey {
    // 将字符串资产类型转换为枚举
    let assetEnum: AssetType;
    
    switch (assetType.toUpperCase()) {
      case 'USDC':
        assetEnum = AssetType.USDC;
        break;
      case 'BNM':
        assetEnum = AssetType.BNM;
        break;
      case 'LINK':
        assetEnum = AssetType.LINK;
        break;
      case 'ETH':
        assetEnum = AssetType.ETH;
        break;
      case 'SOL':
        assetEnum = AssetType.SOL;
        break;
      default:
        assetEnum = AssetType.CUSTOM;
    }
    
    return getAssetAddress(chainId, assetEnum);
  }
   ...
   * @returns 支持的资产列表
   */
  getSupportedAssets(chainId: ChainId): AssetType[] {
    try {
      if (chainId === ChainId.SOLANA_DEVNET) {
        return getCCIPSVMConfig(chainId).supportedAssets;
      } else {
        return getEVMConfig(chainId).supportedAssets;
      }
    } catch (error) {
...
export function useCCIPTransfer() {
  ...
  return {
    ...
    getSupportedAssets: ccipService.getSupportedAssets.bind(ccipService),
