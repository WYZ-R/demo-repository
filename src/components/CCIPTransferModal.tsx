...
const CCIPTransferModal: React.FC<CCIPTransferModalProps> = ({ 
  ...
  // 支持的资产列表
  const [supportedAssets, setSupportedAssets] = useState<AssetType[]>([]);
  
  const { executeTransfer, isWalletConnected, currentChain, getSupportedAssets } = useCCIPTransfer();
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      // 获取支持的资产列表
      const assets = getSupportedAssets(destinationChain);
  ...
  }, [isOpen, initialSourceChain, initialDestinationChain, getSupportedAssets]);
  ...
  useEffect(() => {
    ...
    // 如果当前选择的资产不在支持列表中，重置为第一个支持的资产
    if (assets.length > 0 && !assets.includes(asset as AssetType)) {
      setAsset(assets[0]);
    }
  }, [destinationChain, getSupportedAssets, asset]);
