...
import { chatService, ChatMessage, InvestmentIntent, TransferIntent } from "../services/chatService";
...
const ChatInterface: React.FC = () => {
  ...
  // 最后一次解析的投资意图结果
  const [lastIntent, setLastIntent] = useState<InvestmentIntent | TransferIntent | null>(null);
  ...
  // 转账模态框配置
  const [transferModalConfig, setTransferModalConfig] = useState<{
    sourceChain?: ChainId;
  ...
  const handleSend = async () => {
    ...
    try {
      // 检查是否是转账意图
      const transferIntent = chatService.parseTransferIntent(input);
      
      if (transferIntent && transferIntent.confidence > 0.7) {
        // 如果是转账意图，设置转账模态框配置
        setLastIntent(transferIntent);
        
        // 创建AI回复消息对象
        const aiMessage: ChatMessage = {
          id: (Date.now() + 1).toString(),
          content: `I've detected a transfer request. You want to transfer ${transferIntent.entities.amount} ${transferIntent.entities.asset_type} ${transferIntent.entities.source_chain ? `from ${transferIntent.entities.source_chain}` : ''} ${transferIntent.entities.destination_chain ? `to ${transferIntent.entities.destination_chain}` : ''}. Would you like me to help you execute this transfer?`,
          sender: "ai",
          timestamp: new Date(),
        };
        ...
        // 准备转账模态框配置
        const sourceChain = mapChainFromString(transferIntent.entities.source_chain);
        const destChain = mapChainFromString(transferIntent.entities.destination_chain);
        
        setTransferModalConfig({
          sourceChain,
  ...
  // 将字符串映射到ChainId
  const mapChainFromString = (chainStr?: string): ChainId | undefined => {
    if (!chainStr) return undefined;
    
    const chainLower = chainStr.toLowerCase();
  ...
  const handleExecuteInvestment = () => {
    if (lastIntent && lastIntent.intent === 'invest') {
  ...
  const handleExecuteTransfer = () => {
    if (lastIntent && (lastIntent.intent === 'transfer' || lastIntent.intent === 'send')) {
      // 设置转账模态框配置
      if ('source_chain' in lastIntent.entities || 'destination_chain' in lastIntent.entities) {
        const transferIntent = lastIntent as TransferIntent;
        const sourceChain = mapChainFromString(transferIntent.entities.source_chain);
        const destChain = mapChainFromString(transferIntent.entities.destination_chain);
  ...
  // 渲染投资意图识别卡片的函数
  const renderIntentCard = (intent: InvestmentIntent | TransferIntent) => {
    // 根据意图类型返回对应图标
    const getIntentIcon = (intentType: string) => {
      switch (intentType) {
        ...
        case 'send': return <ArrowRightLeft className="w-5 h-5" />;
    ...
    const getIntentColor = (intentType: string) => {
      switch (intentType) {
        ...
        case 'transfer': return 'from-purple-500 to-blue-500';
        case 'send': return 'from-purple-500 to-blue-500';
        default: return 'from-purple-500 to-blue-500';
      }
    };

    // 判断是否是转账意图
    const isTransferIntent = intent.intent === 'transfer' || intent.intent === 'send';
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center space-x-2">
            ...
            <span className="font-semibold text-slate-800">
              {isTransferIntent ? 'Cross-Chain Transfer Intent' : 'Investment Intent'} Recognition
            </span>
            {/* 置信度标签，根据置信度高低显示不同颜色 */}
          ...
          <div className="flex items-center space-x-2">
            <CCIPTransferButton 
              ...
              sourceChain={
                isTransferIntent && 'source_chain' in entities && entities.source_chain ? 
                mapChainFromString(entities.source_chain) : undefined
              }
              destinationChain={
                isTransferIntent && 'destination_chain' in entities && entities.destination_chain ? 
                mapChainFromString(entities.destination_chain) : 
                'chain' in entities && entities.chain ? 
                (entities.chain.toLowerCase().includes('solana') ? ChainId.SOLANA_DEVNET : ChainId.ETHEREUM_SEPOLIA) : 
                undefined
              }
            />
            
            <button
              onClick={isTransferIntent ? handleExecuteTransfer : handleExecuteInvestment}
  ...
  return (
    <div className="flex h-screen bg-gradient-to-br from-slate-50 to-blue-50">
      ...
      {/* 投资执行模态框 */}
      {lastIntent && lastIntent.intent === 'invest' && (
        <InvestmentExecutionModal
          isOpen={isExecutionModalOpen}
          onClose={() => setIsExecutionModalOpen(false)}
          intent={lastIntent as InvestmentIntent}
        />
      )}
      ...
      <CCIPTransferModal
        ...
        initialSourceChain={transferModalConfig.sourceChain}
        initialDestinationChain={transferModalConfig.destinationChain}
      />
    </div>
  );
};
